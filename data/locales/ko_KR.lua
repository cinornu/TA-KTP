...
t("  · Complete the portal back to the West", "  · 포탈을 타고 서대륙으로 돌아오기", "_t")
t("  · Complete the portal back to the East", "  · 포탈을 타고 동대륙으로 돌아오기", "_t")
t("  · Clear two of the Orc Prides", "  · 오크 긍지 두 곳을 소탕", "_t")
t("  · Clear all of the Orc Prides", "  · 모든 오크 긍지를 소탕", "_t")
t("  · Enter High Peak", "  · 최고봉에 입장", "_t")
t("Semi-Roguelike (barbecue)", "세미-로그라이크 (barbecue)", "birth descriptor name")
t("#GOLD##{bold}#Semi-Roguelike (barbecue) mode#WHITE#", "#GOLD##{bold}#세미-로그라이크 모드 (barbecue)#WHITE#", "_t")
t("  · Clear Dreadfell", "  · 두려움의 영역을 소탕", "_t")
t("  · Clear each of the four Orc Prides", "  · 네 오크 긍지들을 각각 소탕하는데 성공", "_t")
t("Semi-Roguelike (sweet&sour)", "세미-로그라이크 (sweet&sour)", "birth descriptor name")
t("#GOLD##{bold}#Semi-Roguelike (sweet&sour) mode#WHITE#", "#GOLD##{bold}#세미-로그라이크 모드 (sweet&sour)#WHITE#", "_t")
t("  · Defeat Urkis", "  · 우르키스를 패퇴", "_t")
t("  · Rescue Melinda", "  · 멜린다를 구출", "_t")
t("  · Clear the Shadow Crypt", "  · 그림자 지하실을 소탕", "_t")
t("  · Complete the Temple of Creation quest by killing either Slasul or Ukllmswwik", "  · 창조의 사원 퀘스트에서 슬라술 혹은 우클름스윅을 살해", "_t")
t("  · Clear Elven Ruins", "  · 고대 엘프의 폐허를 소탕", "_t")
t("Semi-Roguelike (spicy)", "세미-로그라이크 (spicy)", "birth descriptor name")
t("#GOLD##{bold}#Semi-Roguelike (spicy) mode#WHITE#", "#GOLD##{bold}#세미-로그라이크 모드 (spicy)#WHITE#", "_t")
t("Semi-Roguelike (szechuan)", "세미-로그라이크 (szechuan)", "birth descriptor name")
t("#GOLD##{bold}#Semi-Roguelike (szechuan) mode#WHITE#", "#GOLD##{bold}#세미-로그라이크 모드 (szechuan)#WHITE#", "_t")
t("  · Secure a path to the mainland", "  · 본토로 향하는 길을 확보", "_t")
...
t("Activate the rune to teleport randomly in a range of %d with a minimum range of 15.", "룬을 발동하여 반경 %d 칸 이내의 무작위한 장소로 순간이동합니다. 최소 순간이동 범위는 15 칸 입니다.", "tformat")
...
t("Activate the rune to create a protective shield absorbing at most %d damage for %d turns.", "룬을 발동하여 %d 턴 동안 최대 %d 피해를 흡수하는 보호막을 생성합니다.", "tformat", {2,1})
...
%s]], "tformat")
t("The power released by the %s resonates.", "%s 해방한 힘이 공명.", "tformat", nil, {"이"})
--]==]
...
-- new text
--[==[
t("Semi-Roguelike (mild)", "세미-로그라이크 (mild)", "_t")
t("Semi-Roguelike (barbecue)", "세미-로그라이크 (barbecue)", "_t")
t("Semi-Roguelike (sweet&sour)", "세미-로그라이크 (sweet&sour)", "_t")
t("Semi-Roguelike (spicy)", "세미-로그라이크 (spicy)", "_t")
t("Semi-Roguelike (szechuan)", "세미-로그라이크 (szechuan)", "_t")
t("#GOLD##{bold}#Semi-Roguelike achievements#WHITE##{normal}#", "#GOLD##{bold}#세미-로그라이크 achievements#WHITE##{normal}#", "_t")
t("Semi-Roguelike Mode", "세미-로그라이크 모드", "_t")
...
-- new text
--[==[
t("#GOLD##{bold}#Show negative life in tooltips#WHITE##{normal}#", "#GOLD##{bold}#Show negative life in tooltips#WHITE##{normal}#", "_t")
t("Pragmatic Heroism", "Pragmatic Heroism", "_t")
...
In addition, if enabled, the Minimalist UI's life bar will show the player's negative life value and die-at threshold, with a modified meter display controlled by the "Combined life bar" option below.]], [[If enabled, the tooltips of actors (including the player) with life below zero will show their negative life value and the negative minimum value at which they will die.  If disabled, tooltips will show the game default "HP: unknown".
      
In addition, if enabled, the Minimalist UI's life bar will show the player's negative life value and die-at threshold, with a modified meter display controlled by the "Combined life bar" option below.]], "_t")
t("enabled", "켜짐", "_t")
t("disabled", "꺼짐", "_t")
...
For either setting of this option, if "Show negative life in tooltips" is enabled and your life is below zero, the life bar will show your negative life value and "die-at" threshold numerically.]], [[Controls display of the life bar in the Minimalist and Classic UIs when the "Show negative life in tooltips" option is enabled:

  · If this option is disabled (the default), the life bar will display normally when your life is positive, and when negative will show a black "life" meter expanding from the right as the player's life decreases from zero to your negative die at threshold.

  · If this option is enabled, the life bar will show a "combined" life meter, running from your die-at threshold (zero or negative) up to your max life.  If your die-at threshold is negative, the part of the life bar that reflects negative life will be colored differently.

For either setting of this option, if "Show negative life in tooltips" is enabled and your life is below zero, the life bar will show your negative life value and "die-at" threshold numerically.]], "_t")
t("#GOLD##{bold}#Combined life bar#WHITE##{normal}#", "#GOLD##{bold}#Combined life bar#WHITE##{normal}#", "_t")
t("+", "+", "_t")
--]==]


------------------------------------------------
section "tome-zomnibus/hooks/hooks-inv-keys.lua"


-- new text
--[==[
t("#GOLD##{bold}#Use-item dialog shortcuts#WHITE##{normal}#", "#GOLD##{bold}#Use-item dialog shortcuts#WHITE##{normal}#", "_t")
t("Inventory Keys", "Inventory Keys", "_t")
t([[An interface to configure the shortcut keys used in the use-item dialog.  The configuration dialog lists all the item actions that the game knows about or has seen in use, and allows you to set or clear shortcut keys for each.
      
#YELLOW##{italic}#[DISCLAIMER:  This is an experimental interface.  Some keys may not work as shortcut keys.]#{normal}##LAST#]], [[An interface to configure the shortcut keys used in the use-item dialog.  The configuration dialog lists all the item actions that the game knows about or has seen in use, and allows you to set or clear shortcut keys for each.
      
#YELLOW##{italic}#[DISCLAIMER:  This is an experimental interface.  Some keys may not work as shortcut keys.]#{normal}##LAST#]], "_t")
--]==]
...
t("Landmark:  %s", "Landmark:  %s", "tformat")
t("Action:", "Action:", "_t")
...
t("%s #%s#(min.level: %d)", "%s #%s#(min.level: %d)", "tformat")
...
t("%s (%d)", "%s (%d)", "tformat")
t("Landmarks - %s", "Landmarks - %s", "tformat")
...
-- new text
--[==[
t("#GOLD##{bold}#Low-health warning popup threshold#WHITE##{normal}#", "#GOLD##{bold}#Low-health warning popup threshold#WHITE##{normal}#", "_t")
t("Low Health Popup", "Low Health Popup", "_t")
t("If set to a nonzero value, a warning dialog will pop up when you fall below the specified percentage of your maximum life.  The threshold value can range from 1% to 99%; set to 0 to disable the popup.", "If set to a nonzero value, a warning dialog will pop up when you fall below the specified percentage of your maximum life.  The threshold value can range from 1% to 99%; set to 0 to disable the popup.", "_t")
t("disabled", "꺼짐", "_t")
t("Low Health Popup Threshold", "Low Health Popup Threshold", "_t")
t("% of max life (0 to disable):", "% of max life (0 to disable):", "_t")
t("If enabled, the warning provided by the \"Low-health warning popup threshold\" option will be presented in the style of the in-game \"Life Lost Warning\" option (a prominent message flashed on the screen, and keyboard and mouse input blocked for two seconds) rather than as a popup dialog.", "If enabled, the warning provided by the \"Low-health warning popup threshold\" option will be presented in the style of the in-game \"Life Lost Warning\" option (a prominent message flashed on the screen, and keyboard and mouse input blocked for two seconds) rather than as a popup dialog.", "_t")
t("#GOLD##{bold}#Low-health warning as flyer#WHITE##{normal}#", "#GOLD##{bold}#Low-health warning as flyer#WHITE##{normal}#", "_t")
t("enabled", "켜짐", "_t")
--]==]
...
-- new text
--[==[
t("Meditation is active. Proceed?", "Meditation is active. Proceed?", "_t")
t("No", "아니요", "_t")
t("Yes", "네", "_t")
t("#GOLD##{bold}#Confirm summons while meditating#{normal}##LAST#", "#GOLD##{bold}#Confirm summons while meditating#{normal}##LAST#", "_t")
t("Don't Summon While Meditating", "Don't Summon While Meditating", "_t")
t("If enabled (the default), you will be prompted to confirm using any equilibrium-based summon talent while the Meditation talent is active.", "If enabled (the default), you will be prompted to confirm using any equilibrium-based summon talent while the Meditation talent is active.", "_t")
t("enabled", "켜짐", "_t")
t("disabled", "꺼짐", "_t")
--]==]
...
-- new text
--[==[
t("[Alter the appearance of %s]", "[%s의 모습을 바꾼다]", "tformat")
...
-- new text
--[==[
t("#GOLD##{bold}#Visual Nihil stack markers#{normal}##LAST#", "#GOLD##{bold}#Visual Nihil stack markers#{normal}##LAST#", "_t")
t("Herald of the Herald", "Herald of the Herald", "_t")
...
You will need to restart the game for this change to take effect.]], [[If enabled, targets with the Nihil effect will be marked with a visual indicator of the number of negative magical effects that have been applied to them while Nihil is active, as an aid in summoning a Herald of Oblivion via the Unravel Existence talent.
You will need to restart the game for this change to take effect.]], "_t")
t("enabled", "켜짐", "_t")
t("disabled", "꺼짐", "_t")
--]==]
...
-- new text
--[==[
t("#GOLD##{bold}#Adventurers party response strategy#WHITE##{normal}#", "#GOLD##{bold}#Adventurers party response strategy#WHITE##{normal}#", "_t")
t("Opt-in Adventurers Parties", "Opt-in Adventurers Parties", "_t")
t("Always evade", "Always evade", "_t")
t("Never evade", "Never evade", "_t")
t("Ask", "Ask", "_t")
t("Controls game behavior if you are attacked by adventurers parties or ziguranth patrols (or Allied Kingdoms patrols, for undead) on the world map.  'Ask' (the default) will present a dialog for you to choose to evade or fight the attacker; 'Always evade' and 'Never evade' will choose one of the two options automatically.", "Controls game behavior if you are attacked by adventurers parties or ziguranth patrols (or Allied Kingdoms patrols, for undead) on the world map.  'Ask' (the default) will present a dialog for you to choose to evade or fight the attacker; 'Always evade' and 'Never evade' will choose one of the two options automatically.", "_t")
t("Adventurers party response strategy", "Adventurers party response strategy", "_t")
t("Select strategy", "Select strategy", "_t")
t("Controls game behavior if you are attacked by orc patrols on the world map.  'Ask' (the default) will present a dialog for you to choose to evade or fight the attacker; 'Always evade' and 'Never evade' will choose one of the two options automatically.", "Controls game behavior if you are attacked by orc patrols on the world map.  'Ask' (the default) will present a dialog for you to choose to evade or fight the attacker; 'Always evade' and 'Never evade' will choose one of the two options automatically.", "_t")
t("#GOLD##{bold}#Orc patrol response strategy#WHITE##{normal}#", "#GOLD##{bold}#Orc patrol response strategy#WHITE##{normal}#", "_t")
t("Orc patrol response strategy", "Orc patrol response strategy", "_t")
t("Controls game behavior if you are attacked by Sun Paladin patrols or Anorithil patrols on the world map.  'Ask' (the default) will present a dialog for you to choose to evade or fight the attacker; 'Always evade' and 'Never evade' will choose one of the two options automatically.", "Controls game behavior if you are attacked by Sun Paladin patrols or Anorithil patrols on the world map.  'Ask' (the default) will present a dialog for you to choose to evade or fight the attacker; 'Always evade' and 'Never evade' will choose one of the two options automatically.", "_t")
t("#GOLD##{bold}#Sunwall patrol response strategy#WHITE##{normal}#", "#GOLD##{bold}#Sunwall patrol response strategy#WHITE##{normal}#", "_t")
t("Sunwall patrol response strategy", "Sunwall patrol response strategy", "_t")
t("If this option is set to a nonzero value and any of \"Adventurers party response strategy\", \"Orc patrol response strategy\" or \"Sunwall patrol response strategy\" is set to \"Ask\", then choosing \"Evade\" from the resulting evade-or-fight dialog will temporarily suppress said dialog and evade without prompting for the specified number of turns.  Set to 0 to disable.", "If this option is set to a nonzero value and any of \"Adventurers party response strategy\", \"Orc patrol response strategy\" or \"Sunwall patrol response strategy\" is set to \"Ask\", then choosing \"Evade\" from the resulting evade-or-fight dialog will temporarily suppress said dialog and evade without prompting for the specified number of turns.  Set to 0 to disable.", "_t")
t("#GOLD##{bold}#Extended evade duration#WHITE##{normal}#", "#GOLD##{bold}#Extended evade duration#WHITE##{normal}#", "_t")
t("disabled", "꺼짐", "_t")
t("%d turns", "%d 턴", "tformat")
t("Extended evade duration", "Extended evade duration", "_t")
t("Turns to evade:", "Turns to evade:", "_t")
--]==]
...
-- new text
--[==[
t("#GOLD##{bold}#Alternate zones#WHITE##{normal}#", "#GOLD##{bold}#Alternate zones#WHITE##{normal}#", "_t")
t("Alternate Zones Option", "Alternate Zones Option", "_t")
t("Allow, block or force the appearance of alternate versions of some dungeons and zones.  Alternates can be blocked or forced unilaterally or allowed, blocked or forced individually by zone.  Changing this option will not affect any zones you've already visited with this character.#WHITE#", "Allow, block or force the appearance of alternate versions of some dungeons and zones.  Alternates can be blocked or forced unilaterally or allowed, blocked or forced individually by zone.  Changing this option will not affect any zones you've already visited with this character.#WHITE#", "_t")
--]==]
...
-- new text
--[==[
t("Mark sustains for restart", "Mark sustains for restart", "_t")
t("#GOLD##{bold}#Restart marked sustains while resting#WHITE##{normal}#", "#GOLD##{bold}#Restart marked sustains while resting#WHITE##{normal}#", "_t")
t("Restart Sustains", "Restart Sustains", "_t")
t("If enabled, marked sustains that have been deactivated will be restarted during normal resting as they are with the \"Restart Sustains\" button.", "If enabled, marked sustains that have been deactivated will be restarted during normal resting as they are with the \"Restart Sustains\" button.", "_t")
t("enabled", "켜짐", "_t")
t("disabled", "꺼짐", "_t")
t("Unmark talent for restart", "Unmark talent for restart", "_t")
t("Mark talent for restart", "Mark talent for restart", "_t")
--]==]
...
-- new text
--[==[
t("#GOLD##{bold}#Avoid movement chars in chat dialogs#{normal}##LAST#", "#GOLD##{bold}#Avoid movement chars in chat dialogs#{normal}##LAST#", "_t")
t("Roguelike Chat Guard", "Roguelike Chat Guard", "_t")
t([[Optionally modifies chat dialogs such that any letter key that is bound to a movememt action in your current keybindings will not be assigned as a "shortcut" key for any chat response, to avoid accidentally selecting a chat option when trying to move.  Supported option values are:
  ...
  · "#LIGHT_BLUE#All#LAST#":  A;l chat dialogs are modified.]], [[Optionally modifies chat dialogs such that any letter key that is bound to a movememt action in your current keybindings will not be assigned as a "shortcut" key for any chat response, to avoid accidentally selecting a chat option when trying to move.  Supported option values are:
  · "#LIGHT_BLUE#None#LAST#":  No chat dialogs are modified, as per baseline game behavior.
  · "#LIGHT_BLUE#Some#LAST#":  Certain "high-risk" chat dialogs that are likely to be encountered while the player is moving are modified; others are not.
  · "#LIGHT_BLUE#All#LAST#":  A;l chat dialogs are modified.]], "_t")
t("None", "없음", "_t")
t("Some", "Some", "_t")
t("All", "모든", "_t")
t("???", "???", "_t")
t("Avoid movement chars in chat dialogs", "Avoid movement chars in chat dialogs", "_t")
t("If enabled, no \"shortcut\" keys will be used in chat dialogs.  This supercedes the \"Avoid movement chars in chat dialogs\" option if enabled.", "If enabled, no \"shortcut\" keys will be used in chat dialogs.  This supercedes the \"Avoid movement chars in chat dialogs\" option if enabled.", "_t")
t("#GOLD##{bold}#No hotkeys in chat dialogs#{normal}##LAST#", "#GOLD##{bold}#No hotkeys in chat dialogs#{normal}##LAST#", "_t")
t("enabled", "켜짐", "_t")
t("disabled", "꺼짐", "_t")
--]==]
...
-- new text
--[==[
t("You create: %s", "당신은 %s 생성했다", "logPlayer", nil, {"을"})
t("%s telekinetically seizes: %s.", "%s %s 염동력 손아귀로 끌어옵니다!", "logSeen", nil, {"는","을"})
--]==]
...
-- new text
--[==[
t("Allow/Block Alternate Zones", "Allow/Block Alternate Zones", "_t")
t("Block all alternate zones", "Block all alternate zones", "_t")
t("Force all alternate zones", "Force all alternate zones", "_t")
t("Block/force by zone:", "Block/force by zone:", "_t")
t("#SLATE#[NOTE:  Zones you have not yet visited with any character will not be listed here.]#LAST", "#SLATE#[NOTE:  Zones you have not yet visited with any character will not be listed here.]#LAST", "_t")
t("Block %s", "Block %s", "tformat")
t("Force %s", "Force %s", "tformat")
t("all blocked", "all blocked", "_t")
t("all forced", "all forced", "_t")
t("%d/%d blocked", "%d/%d blocked", "tformat")
t("%d/%d forced", "%d/%d forced", "tformat")
t("%d/%d blocked, %d/%d forced", "%d/%d blocked, %d/%d forced", "tformat")
--]==]
...
t("Corruption/Hexes tree from the Fall of Zigur quest", "Corruption/Hexes tree from the Fall of Zigur quest", "_t")
...
t(" talent", " talent", "_t")
t(" tree", " tree", "_t")
t("%s%s from the %s escort quest", "%s%s from the %s escort quest", "tformat")
...
-- new text
--[==[
t("Character notes", "Character notes", "_t")
...
Drag a talent to add it to or remove it from the restart lists, or to change restart order.]], [[#LIGHT_GREEN#Left-click#LAST# (or press <#LIGHT_GREEN#Enter#LAST#>) on a talent to add it to or remove it from the list of sustains to restart.
#LIGHT_GREEN#Right-click#LAST# (or press '#LIGHT_GREEN#~#LAST#') on a talent to toggle its confirmation status; if any marked talents are active when you click 'Restart Sustains', you will be prompted to confirm the restart.
Drag a talent to add it to or remove it from the restart lists, or to change restart order.]], "_t")
t("Restart Sustains", "Restart Sustains", "_t")
t("#{bold}#Sustains to restart#{normal}#", "#{bold}#Sustains to restart#{normal}#", "_t")
t("#{bold}#Other sustains#{normal}#", "#{bold}#Other sustains#{normal}#", "_t")
t("Talent", "기술", "_t")
t("Status", "상태", "_t")
t("You will be asked to confirm restart if this talent is active.", "You will be asked to confirm restart if this talent is active.", "_t")
t("This sustain will not be restarted until %s becomes active.", "This sustain will not be restarted until %s becomes active.", "tformat")
t("Drop talent here to add it to the list of sustains to restart.", "Drop talent here to add it to the list of sustains to restart.", "_t")
t("Drop talent here to allow it to be restarted before its current \"parent\" (%s).", "Drop talent here to allow it to be restarted before its current \"parent\" (%s).", "tformat")
t("Drop talent here to restart it after %s has been reactivated.", "Drop talent here to restart it after %s has been reactivated.", "tformat")
t("Drop talent here to remove it from the list of sustains to restart.", "Drop talent here to remove it from the list of sustains to restart.", "_t")
t("Active", "켜짐", "_t")
t("Conduit", "Conduit", "_t")
t("On cooldown", "On cooldown", "_t")
t("Unavailable", "불가능", "_t")
t("Drag a talent here from the \"Other sustains\" list to add it to the list of sustains to restart.", "Drag a talent here from the \"Other sustains\" list to add it to the list of sustains to restart.", "_t")
--]==]


------------------------------------------------
section "tome-zomnibus/overload/mod/dialogs/UseItemMnemonicConfig.lua"
...
t("Use-Item Dialog Shortcuts", "Use-Item Dialog Shortcuts", "_t")
t("You can change the shortcut key for a use-item menu action by selecting the action name with arrow keys or the mouse and typing the desired key (or Backspace to clear the key setting).", "You can change the shortcut key for a use-item menu action by selecting the action name with arrow keys or the mouse and typing the desired key (or Backspace to clear the key setting).", "_t")
t("OK", "OK", "_t")
t("Cancel", "취소", "_t")
t("Restore defaults", "Restore defaults", "_t")
t("Action", "행동", "_t")
t("Key", "Key", "_t")
--]==]
...
-- new text
--[==[
t("HP: %d (%d%%) #GREEN#+%0.2f#LAST#", "생명력: %d (%d%%) #GREEN#+%0.2f#LAST#", "tformat")
t("HP: %d/%d (-%d%%) #GREEN#+%0.2f#LAST#", "HP: %d/%d (-%d%%) #GREEN#+%0.2f#LAST#", "tformat")
t("%s (%s)", "%s (%s)", "tformat")
t("Wield/Replace", "Wield/Replace", "_t")
t("Replace which worn/wielded item?", "Replace which worn/wielded item?", "_t")
t("#LIGHT_RED#%s's %s has been deactivated!", "#LIGHT_RED#%s's %s has been deactivated!", "logSeen")
t("You extract %s from %s", "%s 에서 %s 추출했다", "logPlayer", {2,1}, {[2]="를"})
--]==]
...
t("quitting", "종료", "_t")
t("Save and go back to main menu?", "저장하고 메인 메뉴로 나가시겠습니까?", "_t")
t("Save and exit game?", "저장하고 게임을 끝내시겠습니까?", "_t")
--]==]
...
-- new text
--[==[
t("These guys look tough.  Do you want to fight them or evade them?", "These guys look tough.  Do you want to fight them or evade them?", "_t")
t("Evade", "Evade", "_t")
t("Fight", "Fight", "_t")
...
t("#%s#- %s", "#%s#- %s", "tformat")
t("#LIGHT_RED#LOW HEALTH WARNING!", "#LIGHT_RED#LOW HEALTH WARNING!", "say")
t("You have fallen below %d%% of your max life", "You have fallen below %d%% of your max life", "tformat")
t("Low Health!", "Low Health!", "_t")
t("%s's %s", "%s의 %s", "tformat")
t("You now have the ingredients for the following elixirs:", "You now have the ingredients for the following elixirs:", "_t")
...
Having both ZOmnibus and any of its component addons enabled for the same character is very likely to cause errors or damage your character.  We recommend that you return to the Addons dialog and disable either ZOmnibus or the addons listed above.]], [[You have enabled both the ZOmnibus Addon Pack and one or more of its component addons.  In particular, the following conflicting addons are enabled:

  · #YELLOW#%s#LAST#
  
Having both ZOmnibus and any of its component addons enabled for the same character is very likely to cause errors or damage your character.  We recommend that you return to the Addons dialog and disable either ZOmnibus or the addons listed above.]], "tformat")
t("#LIGHT_RED#ZOmnibus Addon Collision#LAST#", "#LIGHT_RED#ZOmnibus Addon Collision#LAST#", "_t")
t("No BG", "No BG", "_t")
t("Grass", "Grass", "_t")
t("Floor", "Floor", "_t")
t("Snow", "Snow", "_t")
...
각각의 능력치는 자연적 최대치인 60 이나, (당신의 레벨에 따라 정해지는) 일정 수준까지만 올릴 수 있습니다.]], "_t")
...
]], [[직업 기술 점수로 새로운 직업 기술을 익히거나, 기존의 직업 기술을 향상시킬 수 있습니다.
직업 기술은 직업의 핵심 능력이며, 따라서 훈련으로 새롭게 익힐 수 없습니다.

레벨이 오를 때마다 1 점의 직업 기술 점수를 얻습니다.
레벨이 5 의 배수가 될 때마다, 직업 기술 점수를 1 점 더 얻을 수 있습니다.
]], "_t")
...
]], [[일반 기술 점수로 새로운 일반 기술을 익히거나, 기존의 일반 기술을 향상시킬 수 있습니다.
일반 기술은 당신의 직업이나 종족에 따라 얻는 것도 있고, 모험을 하는 과정에서 다양한 훈련을 통해 얻을 수도 있습니다.

기본적으로는 레벨이 오를 때마다 1 점의 일반 기술 점수를 얻습니다.
하지만, 레벨이 5 의 배수가 될 때마다 일반 기술 점수를 얻지 못하게 됩니다.
]], "_t")
...
Some races or items may increase them as well.]], [[기술 계열 점수로는 다음 중 하나를 할 수 있습니다:
- 새로운 직업, 일반 기술 계열을 해금합니다.
- 이미 익힌 기술 계열의 숙련도를 0.2 향상시킵니다. (기술 계열당 한 번씩만 가능)
- 각인의 개수를 늘립니다. (각인의 개수는 최대 5개까지)

레벨이 10, 20, 34 가 될 때 기술 계열 점수를 1 점 얻을 수 있습니다.
어떤 종족은 기술 계열 점수를 가지고 시작하며, 희귀하지만 기술 계열 점수를 높여주는 물건도 있습니다.]], "_t")
...
t("Impossible", "불가능", "_t")
t("You cannot learn this talent(s): ", "당신은 이 기술을 배울 수 없다: ", "_t")
